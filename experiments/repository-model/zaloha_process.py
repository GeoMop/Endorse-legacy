...
def generate_fractures(config_dict):
    ...
    fracture_box = [fr_box, 0.5*fr_box, fr_box]
...
def make_mesh(config_dict, fractures, mesh_name, mesh_file):
    ...
    main_tunnel_radius = geom["main_tunnel_radius"]
    main_tunnel_width = geom["main_tunnel_width"]
    main_tunnel_length = geom["main_tunnel_length"]

    small_tunnel_radius = geom["lateral_tunnel_radius"]
    small_tunnel_width = geom["lateral_tunnel_width"]
    small_tunnel_length = geom["lateral_tunnel_length"]
    small_tunnel_passage_width = geom["lateral_tunnel_passage_width"]

    borehole_radius = geom["borehole_radius"]
    borehole_length = geom["borehole_length"]
    ...
    # main tunnel
    y_shift = math.sqrt(main_tunnel_radius * main_tunnel_radius - 0.25 * main_tunnel_width * main_tunnel_width)\
              - main_tunnel_radius / 2
    main_tunnel_block = factory.box([main_tunnel_width, main_tunnel_radius, main_tunnel_length])
    main_tunnel_block_tmp = main_tunnel_block.copy().translate([0, main_tunnel_radius, 0])
    main_tunnel_cylinder_tmp = factory.cylinder(main_tunnel_radius, axis=[0, 0, main_tunnel_length])\
        .translate([0, -y_shift, -main_tunnel_length / 2])
    main_tunnel_cylinder = main_tunnel_block_tmp.intersect(main_tunnel_cylinder_tmp)\

    # # lateral part of main tunnel
    # y_shift_2 = math.sqrt(small_tunnel_radius * small_tunnel_radius - 0.25 * small_tunnel_width * small_tunnel_width)\
    #             - small_tunnel_radius / 2
    #
    # small_tunnel_block = factory.box([small_tunnel_length, small_tunnel_radius, small_tunnel_width])
    # small_tunnel_block_tmp = small_tunnel_block.copy().translate([0, small_tunnel_radius, 0])
    # small_tunnel_cylinder_tmp = factory.cylinder(small_tunnel_radius, axis=[small_tunnel_length, 0, 0])\
    #     .translate([-small_tunnel_length / 2, -y_shift_2, 0])
    ...
    #     .translate([small_tunnel_length / 2, 0, 0])
    # lateral_tunnel_part_2 = lateral_tunnel_part_1.copy().translate([0, 0, 20])
    # lateral_tunnel_part = factory.group(lateral_tunnel_part_1, lateral_tunnel_part_2)

    # main tunnel with lateral parts
    left_well = factory.group(main_tunnel_block, main_tunnel_cylinder).translate([-30, 0, 0])  #, lateral_tunnel_part)\

    # horizontal boreholes
    right_well_1 = factory.cylinder(borehole_radius, axis=[borehole_length, 0, 0])\
        .translate([small_tunnel_length, 0, 0])
        # .translate([small_tunnel_length / 2 + 1, 0, 0])
