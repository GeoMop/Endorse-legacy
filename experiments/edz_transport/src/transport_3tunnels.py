...
def generate_fractures(config_dict):
    ...
    pop.set_sample_range([1, fr_box], sample_size=geom["n_frac_limit"])
...
def prepare_mesh(config_dict, fractures):
    ...
    geom_dict = config_dict["geometry"]
    mesh_file = mesh_name + ".msh"
    do_mesh = not os.path.isfile(mesh_file) #or True
    if do_mesh:
        mesh.make_mesh(geom_dict, fractures, mesh_file)
        #test_field.apply_field4(config_dict, fractures, mesh_name, mesh_file)

    # Fracture regions maps

    mesh_healed = mesh_name + "_healed.msh"
    do_heal = not os.path.isfile(mesh_healed) # or True


    hm = heal_mesh.HealMesh.read_mesh(mesh_file, node_tol=1e-4)

    own_name_to_id = {fr.region.name: fr.region.id for fr in fractures}
    own_to_gmsh_id = {own_name_to_id[name]: gmsh_id for name,(gmsh_id, dim) in hm.mesh.physical.items() if name in own_name_to_id}
    #own_to_gmsh_id = {own: gmsh for gmsh, own in gmsh_to_own_id.items()}

    new_reg_id = max( [gmsh_id for gmsh_id, dim in hm.mesh.physical.values()] ) + 1
    ...
    elm_to_fr = {elm: reg_to_fr[reg_id] for elm, (reg_id, dim) in elm_to_orig_reg.items()}
    return mesh_healed, elm_to_fr
...
def set_source_limits(config_dict):
    geom = config_dict["geometry"]
    shift_x, shift_y, _  = geom["shift_vec"]
    br = geom["borehole_radius"]


    trans_params = config_dict["transport_params"]
    src_ipos = trans_params['source_ipos'] # position of the leaking container [m] from beginning of the borehole
    src_length = trans_params['source_length'] # length of the container
    src_space = trans_params['source_space']  # space between containers

    x_pos = shift_x + src_ipos * (src_length + src_space)
    trans_params['source_y0'] = shift_y - 2 * br
    trans_params['source_y1'] = shift_y + 2 * br
    trans_params['source_x0'] = x_pos
    trans_params['source_x1'] = x_pos + src_length

def sample(config_dict, seed):
    sample_dir=os.path.join(config_dict["sample_dir"], f"seed_{seed}")
    with tool.workdir(sample_dir):
        # copy files
        for g in config_dict["copy_files"]:
            shutil.copyfile(os.path.join(script_dir, g), g)
        # flow_bin path
        ...
        config_dict["_aux_flow_path"] = config_dict["flow_executable"]
        ...
        healed_mesh, elem_to_fr = prepare_mesh(config_dict, fractures)
        healed_mesh_bn = os.path.basename(healed_mesh)
        config_dict["transport_params"]["mesh_file"] = healed_mesh_bn

        set_source_limits(config_dict)

        # compute GMSH file with elementwise conductivity and cross-section
        field_interpolate(config_dict, elem_to_fr)
        transport_succeed = call_flow(config_dict, 'transport_params', result_files=["mass_balance.yaml"])
        if transport_succeed:
            print("all fine")
...
def untar_large_model():
    large_model_path = "large_model.msh"
    in_path = "large_model.tar.gz"
    if not os.path.isfile(large_model_path):
        print("Extracting the large_model.msh. Takes few seconds.")
        subprocess.run(['tar', '-xf', in_path])
...
if __name__ == "__main__":
    ...
    #mesh.make_mesh(config_dict, fractures, mesh_name="geom_komplet", mesh_file="final_mesh.msh")
    ###############################################################################################

    sample(config_dict, seed=102)
