...
stanovení. Ve spolupráci s~aplikačním garantem pak předpokládáme upřesnění 
...
{\bf  Akustické emise} v okolí EDZ (ve vrtech) je trvale instalována soustava citlivých geofonů a kontinuálně snímá přirozené mikroseismické události. Časy příchodu umožnují prostorovou lokalizaci událostí. Z jejich rozložení lze usuzovat na rozsah EDZ.
...
Tato kapitola obsahuje rámcový návrh experimentu inspirovaného pracemi \cite{Rutqvist2004} (FEBEX, Grimsel, Švýcarsko) a
...
poměrů v okolí nově vyraženého díla. Podrobněji popíšeme měření a modely použité v druhé práci v rámci rozsáhlého experimentu TSX 
, \cite{Chandler2002}.

Měření probíhalo v horizontálním vrtu HGT2 a vertikálním vrtu HGT1 oba vrty byly vyraženy z dříve existujících chodeb tak, aby směřovaly do středu nově raženého úseku a to kolmo na jeho osu. Z většího množství měřících sond byly použity pouze signály ze čtyř sond ve vzdálenostech 1.5m a 4m od stěny vyražené chodby (viz. schéma v Obr. \ref{fig:TSX}) ostatní sondy vykazovaly výrazně nižší změny tlaku v reakci na ražbu. Výhodou kontinuálního měření získání většího množství dat i s malým množstvím senzorů. 

V rámci projektu DECOVALEX-THMC project (2004–2007), Task A bylo toto měření modelováno čtyřmi týmy s použitím různě složitých hydro-mechanických 
...
Zdá se však, že další měření zejména výsledky MVP a SEPPI byla použita pouze k počítečním odhadům mechanických a hydraulických parametrů neporušené horniny.


\subsubsection{Závěry}
\begin{itemize}
 \item Modely projektu Endorse lze částečně validovat vůči existujícím datům z projektu TSX, za předpokladu možnosti jejich získání. 
 Po zopakování výše uvedených výsledků lze provést stochastické výpočty pro zahrnutí vlivu nejistot a ověřit citlivost měření vůči parametrům modelu.
 Následně lze provést porovnání s víceškálovým přístupem.
 \item Uvedené měření i modely byly realizovány v jednom řezu kolmo na osu díla není tak možno predikovat anizotropii EDZ s výrazně vyšší vodivostí ve směru parallelním s osou díla. Pro postižení této anisotropie by bylo nutné realizovat měření ve vrtech umístěných na různých metrážích raženého díla, nebo i těsně před ním. Předchozí modelování by mělo teoreticky ověřit citlivost takového postupu vůči anisotroii permeability.
 \item Výše uvedené měření vyžadovalo sondy ve speciálně situavaných vrtech. V případě realizace podobného experimentu na PVP Bukov lze ověřit
