    ...
    for rho in rho_list:
        ...
        while True:
            ...
            if os.path.exists(dir_name):
                ...
                results.setdefault(rho, []).append(K_eff - 1)
...
def plot_results(results):
    ...
    for rho, k_eff in results.items():
        axes.scatter(rho * np.ones(len(k_eff)), k_eff)

    axes.set_yscale('log')
    axes.set_ylabel(r"$K^{\prime}_{eff} - 1$")
    axes.set_xlabel(r"$\rho^{\prime}_{2}$")

    fig.savefig("conductivity_sigma_100")
    fig.show()
...
def analyze_results(seeds_results):
    ...
    matplotlib_variogram_plot(Vnorth_variograms, hist=False, show=True,
                              title="{}_{}".format(seed, "north_diff_squares"))
...
def run_multiple_samples(work_dir, sample_dict, n_samples=10):
    ...
    work_dir = os.path.join("/home/martin/Documents/Endorse/ms-homogenization/seed_26339/comparison/sigma_10/LMH/",
                            "rho_1")
    work_dir = "/home/martin/Documents/Endorse/ms-homogenization/test_regions"
