...
def write_fields(mesh, basename, bulk_model, fracture_model):
    ...
    for el_id, ele in gmsh_mesh_bulk_elements(mesh):
        ...
        if n_nodes == 2:
            ...
            print("cond tn fracture ", cond_tn)
        else:
            # n_nodes == 3
...
class FlowProblem:
    ...
    @classmethod
    def make_fine(cls, i_level, fr_range, fractures, finer_level_path, config_dict, out_dir=None):
        ...
        return FlowProblem(i_level, "fine" if out_dir is None else out_dir,
...
class BothSample:
    ...
    def calculate(self, out_dir=None):
...
def compute_variogram(cond_field_xy, scalar_cond_log, maxlag=800, dir=None):
    ...
    V1 = skg.Variogram(cond_field_xy_arr, scalar_cond_log_arr, maxlag=maxlag)
    ...
    fig, ax = plt.subplots(1, 1, figsize=(8, 6))
    Vnorth = skg.DirectionalVariogram(cond_field_xy_arr, scalar_cond_log_arr, maxlag=maxlag, azimuth=90, tolerance=90)
    ...
    fig, ax = plt.subplots(1, 1, figsize=(8, 6))
    Veast = skg.DirectionalVariogram(cond_field_xy_arr, scalar_cond_log_arr, maxlag=maxlag, azimuth=0, tolerance=90)
...
def analyze_results(seeds_results):
    ...
    for seed, subdir_results in seeds_results.items():
        ...
        for subdir, (cond_field_xy, scalar_cond_log, fractures_cond, bulk_cond) in subdir_results.items():
            ...
            seed_hist_data.append((scalar_cond_log, fractures_cond, bulk_cond))
...
def plot_histogram(hist_data):
    ...
    for (scalar_cond_log, fractures_cond, bulk_cond), axes in zip(hist_data, axs):
        axes.hist(scalar_cond_log, density=True, label="eq conductivity")
        axes.hist(fractures_cond, density=True, label="fractures cond")
        axes.hist(bulk_cond, density=True, label="bulk cond")
...
def run_samples(work_dir, sample_dict):
    ...
    work_dir = os.path.join("/home/martin/Documents/Endorse/ms-homogenization/seed_12345", "n_10_s_100_100_step_5")
